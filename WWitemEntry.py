...
class Ui_addItemWindow(object):
    ...
    def submit(self):
        if self.widthEntry.text().isnumeric() == False or self.itemNameEntry.text() == "" or self.heightEntry.text().isnumeric() == False:
            ...
            formData = [self.itemNameEntry.text(), self.heightEntry.text(), self.widthEntry.text(), self.descriptionEntry.text(), self.categoryEntry.text()]
            f = open("temp.bin", 'wb')
            dump(formData, f)
            f.close()
            exit(0)
